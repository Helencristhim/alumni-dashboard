...
export default function ContratosB2CPage() {
  const [contracts, setContracts] = useState<B2CContractData[]>([]);
  ...
  const fetchContracts = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/b2c-contracts?${params}`);
  ...
  const handleDuplicate = async (id: string) => {
    try {
      await fetch(`/api/b2c-contracts/${id}/duplicate`, { method: 'POST' });
      fetchContracts();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/b2c-contracts/${id}`, { method: 'DELETE' });
      fetchContracts();
    } catch (err) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Contratos B2C</h1>
            <p className="text-gray-500 mt-1">
              Gestão de contratos individuais de alunos
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <form onSubmit={handleSearch} className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Buscar por aluno, CPF ou número..."
            ...
            <div className="flex gap-2">
              <div className="relative">
                ...
                >
                  ...
                  {Object.entries(B2C_CONTRACT_STATUS_LABELS).map(([value, label]) => (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-500">Aluno</th>
                  ...
                  contracts.map((contract) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        <Link
                          href={`/contratos-b2c/${contract.id}`}
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {contract.cpfAluno && (
                              <p className="text-xs text-gray-500">{contract.cpfAluno}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-gray-700">
                        {B2C_CONTRACT_TYPE_LABELS[contract.type as B2CContractType] || contract.type}
                      </td>
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            B2C_CONTRACT_STATUS_COLORS[contract.status as B2CContractStatus] || ''
                          }`}
                        >
                          {B2C_CONTRACT_STATUS_LABELS[contract.status as B2CContractStatus] ||
                            contract.status}
                        </span>
                      </td>
