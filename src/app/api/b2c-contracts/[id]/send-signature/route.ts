...
// POST /api/b2c-contracts/[id]/send-signature - Enviar para assinatura via ZapSign
  ...
  try {
    ...
    if (!signatories?.length) {
      return NextResponse.json(
        { success: false, error: 'Informe pelo menos um signatario' },
        { status: 400 }
      );
    }
    ...
    if (contract.status === 'SIGNED') {
      return NextResponse.json(
        { success: false, error: 'Contrato ja foi assinado e nao pode ser reenviado' },
        { status: 400 }
      );
    }
    ...
    const docName = `${contract.title} - ${contract.nomeAluno}`;
    ...
    // 4. Salvar signatarios e atualizar contrato em transacao
    const updated = await prisma.$transaction(async (tx) => {
      // Limpar signatarios anteriores (permite reenvio)
      await tx.b2CContractSignatory.deleteMany({
        where: { contractId: id },
      });

      for (let i = 0; i < signatories.length; i++) {
        const zapSigner = docResult.signers[i];
        await tx.b2CContractSignatory.create({
          data: {
            contractId: id,
            name: signatories[i].name,
            email: signatories[i].email,
            cpf: signatories[i].cpf ?? '',
            role: signatories[i].role || 'aluno',
            status: 'pending',
    ...
    console.error('Erro ao enviar contrato B2C para assinatura:', error);
    const message =
